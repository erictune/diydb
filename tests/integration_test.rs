...
fn test_run_query_on_multipage_with_various_page_sizes() {
    ...
    for db in dbs {
        ...
        assert_eq!(qot.rows[0].items, vec![Text(String::from("A")), Text(String::from("A")), Text(String::from("A"))]);

        assert_eq!(qot.rows[284].row_id, 285);
        assert_eq!(qot.rows[284].items.len(), 3);
        assert_eq!(qot.rows[284].items, vec![Text(String::from("C")), Text(String::from("I")), Text(String::from("E"))]);

        assert_eq!(qot.rows[999].row_id, 1000);
        assert_eq!(qot.rows[999].items.len(), 3);
        assert_eq!(qot.rows[999].items, vec![Text(String::from("J")), Text(String::from("J")), Text(String::from("J"))]);
    }
}
