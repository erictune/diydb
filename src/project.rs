...
pub fn build_project(in_colnames: &Vec<String>, in_coltypes: &Vec<SqlType>, out_cols: &Vec<ast::SelItem>) -> Result<(Vec<ProjectAction>, Vec<String>, Vec<SqlType>)> {
    let mut actions = vec![];
        ...
        match out_item {
            ...
            ast::SelItem::ColName(n) => {
                let idx: usize = match input_indexes.get(n.name.as_str()) {
                    Some(idx) => *idx,
                    None => panic!("Column name not found: {} not in {:?}", n, input_indexes.keys()),
                };
...
fn test_build_project_colnames_only() {
    ...
    let expected_colnames: Vec<String> = vec!["a", "c", "b", "a"].iter().map(|i| String::from(*i)).collect();
