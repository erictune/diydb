...
fn test_interior_iterator_on_multipage_db() {
    ...
    let mut pager = crate::pager::Pager::open(path.as_str()).expect("Should have opened pager for db {path}.");
    pager.initialize().expect("Should have initialized pager for db {path}.");
